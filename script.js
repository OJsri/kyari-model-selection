...
async function loadKyariDatabase() {
    try {
        const response = await fetch('Final_approved_kyari_13aug.csv');
...
function getKyariForCalculation() {
    ...
    return {
        ...
        area: parseFloat(selectedKyari.Total_Area), // in acres - from CSV data
        perimeter: parseFloat(selectedKyari['Perimeter m']), // in meters - from CSV data
        areaInSqMeters: parseFloat(selectedKyari['Area m^2']) // in square meters - from CSV data
    };
}
